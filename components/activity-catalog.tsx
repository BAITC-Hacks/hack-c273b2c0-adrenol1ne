...
import type { Activity, Candidate, Snapshot } from "@/lib/types";
import {
  rankActivities,
  requirementsFor,
  scoreActivity,
...
export function ActivityCatalog({
  ...
  const requirements =
    career === "all"
      ? state.requirements
      : requirementsFor(profile, state.requirements);
  const recommended = rankActivities(
    profile,
    state.events,
    state.requirements,
    state.skills,
    ...
    event.gains.some((g) =>
      requirements.some(
        (r) =>
          r.skillId === g.skillId &&
          r.requiredLevel >
            (profile.skills.find((s) => s.skillId === g.skillId)?.level ?? 0) &&
          g.maxLevel >
            (profile.skills.find((s) => s.skillId === g.skillId)?.level ?? 0),
      ),
    );
  const selected = state.events
    .filter((event) => {
      ...
      return (
        ...
        (career === "all" ||
          event.gains.some((g) =>
            requirements.some((r) => r.skillId === g.skillId),
          ))
      );
    })
    .sort((a, b) =>
