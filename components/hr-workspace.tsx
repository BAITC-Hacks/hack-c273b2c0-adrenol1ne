...
import { simulateWorkforce } from "@/lib/scenario-engine";
import { Shell } from "./shell";
import { Badge, Empty, Heading, SectionHeading } from "./shared";
...
export function HrWorkspace({
  ...
  const [search, setSearch] = useState(""),
    ...
    [scenario, setScenario] = useState<ReturnType<
      typeof simulateWorkforce
    > | null>(null),
  ...
  const close = data.employees.filter(
      (e) => e.readiness >= 80 && e.readiness < 100,
    ).length,
    uncovered = data.employees.filter((e) => !e.recommendationCount).length;
  const people = data.employees
    ...
    .filter(
      (e) =>
        ...
        (employeeFilter === "Near target" && e.readiness >= 80) ||
                ...
                <>
                  ...
                  {[
                    ...
                    <div className="scenario-result-row" key={row.label}>
                      ...
                      <span>{row.label}</span>
                      <strong>{row.n}</strong>
                      <Progress value={(row.n / scenario.needed) * 100} />
                    </div>
                  ))}
                  <div className="scenario-summary">
                    <strong>
                      {Math.round(
                        (1 - scenario.external / scenario.needed) * 100,
                      )}
                      %
                    </strong>
                    <div>
