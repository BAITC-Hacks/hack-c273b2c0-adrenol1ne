...
async function ensureDemoManager() {
  ...
  if (employee)
    await db.managerTeam.upsert({
      where: {
        managerId_department: {
          managerId: "DEMO-MANAGER",
          department: employee.department,
        },
      },
      create: { managerId: "DEMO-MANAGER", department: employee.department },
      update: {},
    });
}
async function main() {
  if (await db.employee.count()) {
    ...
    const { ensureLearningPaths } = await import(
      "@backend/services/learning-content.service"
    );
