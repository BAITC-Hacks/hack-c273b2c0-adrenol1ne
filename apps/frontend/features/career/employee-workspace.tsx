...
} from "@frontend/features/activities/activity-catalog";
import { SkillEvidencePanel } from "@frontend/features/skills/skill-evidence";
...
export function EmployeeWorkspace({
  ...
  const hrView = !access.canEdit;
  const employeeBase = access.basePath;
  const employee = state.employee,
    req = state.career.requirements,
    ready = state.career.readiness,
    best = state.recommendations[0],
    target = `${employee.targetGrade} ${employee.targetRole}`;
  const gaps = state.career.gaps;
  ...
  async function startActivity(eventId: string) {
    ...
    setError("");
    try {
      const response = await apiFetch("/api/learning/start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ activityId: eventId }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(apiErrorMessage(result.error));
  ...
  async function activityAction(
    ...
    try {
      ...
      if (!response.ok) throw new Error(apiErrorMessage(data.error));
      setState(data.state);
      setSelected(null);
      setToast(
  ...
  async function changeTarget(role: string, grade: string) {
    ...
    setError("");
    try {
      const r = await apiFetch("/api/target", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function activityCard(activity: Activity, index?: number) {
    const candidate = state.activityCandidates.find(
    ...
    return (
      <div className={`skill-table ${compact ? "compact" : ""}`}>
        ...
        {(compact
          ...
          .filter(Boolean)
          .map((s) => {
            const { level, required, gap } = state.skillDetails[s.id];
            return (
              <button
                key={s.id}
  ...
  const content = (
    <>
      {hrView && (
        <Link
          href={access.role === "MANAGER" ? "/manager" : "/hr/employees"}
          className="text-link"
        >
          ← Back to employees
        </Link>
            ...
            {state.routes.map((route) => {
              ...
              return (
                ...
                >
                  ...
                  <p>
                    {route.gapSkillIds.map(skillName).join(" · ") ||
                      "All current skill requirements met"}
                  </p>
                  <Button
            ...
            <>
              <div className="skills-summary">
                ...
                <section className="card skill-summary-card">
                  ...
                  <ReadinessRing value={ready} />
                  <h2>
                    {state.career.metCount} of {req.length} requirements met
                  </h2>
                  <p>
        ...
        <>
          ...
          <div className="mission-grid">
            {state.missions.map((m) => {
              ...
              const matched = state.missionMatches[m.id];
              return (
                  ...
                  <div className="mission-body">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        event &&
                        explain(
                          state.activityCandidates.find(
                            (candidate) => candidate.activity.id === event.id,
                          )!,
                        )
                      }
                    >
                ...
                <div className="why-not-content">
                  ...
                  <div className="compare-scores">
                    ...
                    {(() => {
                      ...
                      const alt = state.activityCandidates.find(
                        (candidate) => candidate.activity.id === speaking.id,
                      )!;
                      return (
                        <div>
      ...
      <Dialog open={requirementsOpen} onOpenChange={setRequirementsOpen}>
        <DialogContent>
          ...
          <DialogDescription>
            {state.career.criticalGapCount} critical skill gaps are currently
  ...
  return hrView ? (
    <Shell
      role={access.role}
      name={access.role === "MANAGER" ? "Team Manager" : "People & Culture"}
    >
      {content}
    </Shell>
  ) : (
