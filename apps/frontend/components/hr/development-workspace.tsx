...
export function DevelopmentWorkspace({
  ...
  async function refresh() {
    const response = await apiFetch("/api/hr/development", {
      cache: "no-store",
    });
    const result = await response.json();
    if (!response.ok)
      throw new Error(
        apiErrorMessage(result.error, "Unable to refresh development data."),
      );
  ...
  async function generate() {
    ...
    setReviewed(false);
    try {
      const response = await apiFetch("/api/hr/development/draft", {
        method: "POST",
  ...
  async function publish() {
    ...
    setError("");
    try {
      const response = await apiFetch("/api/hr/development/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function approve(
    ...
    setNotice("");
    try {
      const response = await apiFetch("/api/hr/development/approve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok)
        throw new Error(
          apiErrorMessage(result.error, "Could not save the mentor decision."),
        );
      setData(result as DevelopmentReport);
      setNotice(
        approved
      ...
      <div className="learning-workspace">
        ...
        <div className="dev-summary">
          ...
          <div className="dev-kpi">
            ...
            <small>
              Employees with a verified completion:{" "}
              {data.metrics.employeesCompleted}
            </small>
          </div>
          <div className="dev-kpi">
            <span>
