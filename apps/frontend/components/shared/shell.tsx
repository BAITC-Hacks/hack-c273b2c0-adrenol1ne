...
import { Brand } from "@frontend/components/shared/presentation";
...
export function Shell({
  ...
  const nav =
    role === "HR"
      ? hrNav
      : role === "MANAGER"
        ? [{ href: "/manager", name: "My team", icon: Users }]
        : employeeNav;
  const current = nav.find((n) => n.href === path)?.name ?? "Talent workspace";
  async function switchRole(
    nextRole: UserRole = role === "HR" ? "EMPLOYEE" : "HR",
  ) {
    setBusy(true);
    setError("");
    try {
      ...
      if (!r.ok) throw new Error(apiErrorMessage(d.error));
  ...
  return (
    <div className="app-shell">
      ...
      <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
        <Link href={ROLE_HOME[role]} aria-label="Halyk TalentOS home">
          <Brand />
        </Link>
        <button
